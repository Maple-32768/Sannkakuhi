...
public class Sankakuhi {
	...
	String[] type = {"sin","cos","tan","cosec","sec","cot"},
			sin = {"0",null,"1/2","1/Å„2","Å„3/2",null,"1"},
			cos = {"1",null,"Å„3/2","1/Å„2","1/2",null,"0"},
			tan = {"0",null,"1/Å„3","1","Å„3",null,"Ç»Çµ"},
			cosec = {"Ç»Çµ",null,"2","Å„2","2/Å„3",null,"1"},
			sec = {"1",null,"2/Å„3","Å„2","2",null,"Ç»Çµ"},
			cot = {"Ç»Çµ",null,"Å„3","1","1/Å„3",null,"0"};
	...
	String Answer(String type, int angle) {
		...
		if(angle >= 180) {
			angle-=180;
			if(type != this.type[2] && type != this.type[5]) {	//tan,cotà»äO
				minus_i += 1;
			}
		}
		...
		if(angle > 90) {
			angle = 180-angle;
			if(type != this.type[0] && type != this.type[3]) {	//sin,cosecà»äO
				minus_i += 1;
			}
		}
